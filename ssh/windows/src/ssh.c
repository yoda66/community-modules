...
// {72dd5950-b684-11ec-9b21-8b2a13dd1ff5}
static const GUID sshModuleId =
{ 0x72dd5950, 0xb684, 0x11ec, { 0x9b, 0x21, 0x8b, 0x2a, 0x13, 0xdd, 0x1f, 0xf5}};
...
__checkReturn __success(return == SCYTHE_NOERROR) INT init(__in PSCYTHE_CLIENT pClient, __in_opt PVOID pvReserved)
{
    ...
    DBGPRINT("ssh::init function called.\n");
...
__checkReturn __success(return == SCYTHE_NOERROR) ERROR_T getinfo(__inout PSCYTHE_MODULE_INFORMATION pInformation)
{
    ...
    DBGPRINT("ssh::getinfo function was called.");
...
__checkReturn __success(return == SCYTHE_NOERROR) ERROR_T deinit(__in PVOID pvReserved)
{
    ...
    DBGPRINT("ssh::deinit function was called.");
    return SCYTHE_NOERROR;
}
...
BOOL WINAPI DllMain(__in HINSTANCE hinstDLL, __in DWORD fdwReason, __in LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        DBGPRINT("ssh: DLL_PROCESS_ATTACH");

        break;
    case DLL_PROCESS_DETACH:
        DBGPRINT("ssh: DLL_PROCESS_DETACH");
        break;
    default:
        break;
    }
...
void scp_write_file(struct ssh_sesh* sesh, const char* file, const char* filename) {
    ...
    rc = ssh_scp_push_file(sesh->scp, filename, strlen(file), _S_IREAD | _S_IWRITE| O_CREAT);
    if (rc != SSH_OK) {
...
void transfer_file_sftp(struct ssh_sesh* sesh, const char* filebytes, const char* filename) {
    ...
    file = sftp_open(sesh->sftp, filename, access_type, S_IWRITE);
    if (file == NULL) {
